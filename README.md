...
Each entry becomes its own source in the picker:
...
An account is listed by its handle, which is what you typed. A feed is listed by its own name once it has been asked for one - until then, by the last part of its URI.
...
Latest post and Latest reply are messages, and go on a **Notifications** page as a block of text with who it is from and how long ago. The rest are numbers and go anywhere a number goes.
...
**Replies stand in for mentions.** Notifications need a login and post search is refused without one, so the threads under an account's own posts are the only public place a mention can come from. Up to three are opened, newest first, and the account's own replies do not count - a thread somebody is carrying on alone is not somebody answering.
